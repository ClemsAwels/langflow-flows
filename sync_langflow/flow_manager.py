        ...
        for flow_path in added_flows:
            try:
                ...
                if not os.path.exists(full_path):
                    logger.warning(f"Le fichier {full_path} n'existe pas")
                    continue
                ...
                if success and flow_id and flow_data:
                    ...
                    logger.error(f"Échec de l'ajout du flow {flow_path}: {flow_id}")
            except Exception as e:
                logger.error(f"Erreur lors du traitement du flow ajouté {flow_path}: {e}")
    ...
    def process_modified_flows(self, modified_flows: List[str], repo_path: str) -> Dict[str, Dict[str, Any]]:
        ...
        for flow_path in modified_flows:
            try:
                ...
                flow = self.find_flow_by_name(flow_name)
                if not flow:
                    logger.warning(f"Flow '{flow_name}' non trouvé, tentative d'ajout à la place")
                ...
                if success and flow_id and updated_flow:
                    ...
                    logger.error(f"Échec de la mise à jour du flow {flow_path}: {flow_id}")
            except json.JSONDecodeError as e:
                logger.error(f"Erreur de décodage JSON pour le fichier {flow_path}: {e}")
            except Exception as e:
                logger.error(f"Erreur lors du traitement du flow modifié {flow_path}: {e}")
