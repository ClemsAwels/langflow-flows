...
class GitManager:
    ...
    def log_changes(self, changes: Dict[str, List[str]], detailed: bool = False) -> None:
        """
        Enregistre dans le logger toutes les modifications détectées.
        
        Args:
            changes: Dictionnaire contenant les listes de fichiers ajoutés, modifiés et supprimés.
            detailed: Si True, affiche les chemins complets des fichiers (mode détaillé).
        """
        logger.info(f"=== Récapitulatif des changements Git ===")
        logger.info(f"Fichiers ajoutés: {len(changes['added'])}")
        logger.info(f"Fichiers modifiés: {len(changes['modified'])}")
        logger.info(f"Fichiers supprimés: {len(changes['deleted'])}")
        ...
        if detailed:
            if changes['added']:
                logger.debug("=== Fichiers ajoutés ===")
                for file in changes['added']:
                    logger.debug(f" - {file}")
            
            if changes['modified']:
                logger.debug("=== Fichiers modifiés ===")
                for file in changes['modified']:
                    logger.debug(f" - {file}")
            
            if changes['deleted']:
                logger.debug("=== Fichiers supprimés ===")
                for file in changes['deleted']:
                    logger.debug(f" - {file}")
            
            if changes['flows_added']:
                logger.debug("=== Flows ajoutés ===")
                for file in changes['flows_added']:
                    logger.debug(f" - {file}")
            
            if changes['flows_modified']:
                logger.debug("=== Flows modifiés ===")
                for file in changes['flows_modified']:
                    logger.debug(f" - {file}")
            
            if changes['flows_deleted']:
                logger.debug("=== Flows supprimés ===")
                for file in changes['flows_deleted']:
                    logger.debug(f" - {file}")